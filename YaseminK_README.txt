...
Then specify the png file to announce the chunks via broadcast.
Secondly run Chunk_Discovery.py,
By listening the broadcast of Chunk_Announcer.py discover the chunks and the IP addresses to request them from. The discovering 
loop is forever so you can stop running after any repetitions occured. 
...
This will help you to start a TCP connection and get the requested contents before sending them. Meanwhile sending a chunk, 
take a look at uploadLog.txt you've created to see the chunk names, timestamps and destination IP address in more detailed way.
...
Then in line 47 and 48, change "255.255.255.255" to "25.255.255.255" in order to broadcast.
Then follow the localhost guide or the following steps:
